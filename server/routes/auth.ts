...
export function createAuthRoutes(pool: Pool, ensureVisitor: ReturnType<typeof createEnsureVisitor>) {
  ...
  router.post('/auth/signup', ensureVisitor, async (req: AuthRequest, res: Response) => {
    try {
      ...
      await pool.query('DELETE FROM observe_events WHERE user_id = $1 AND source = $2', [req.visitorId, 'sample'])
      await pool.query('DELETE FROM customers WHERE user_id = $1', [req.visitorId])
      await pool.query('DELETE FROM subscriptions WHERE user_id = $1', [req.visitorId])
      await pool.query('DELETE FROM plans WHERE user_id = $1', [req.visitorId])
  ...
  // ─── Forgot password ──────────────────────────────────────────────────
  router.post('/auth/forgot-password', async (req: AuthRequest, res: Response) => {
    try {
      ...
      const normalizedEmail = email.toLowerCase().trim()
      const result = await pool.query('SELECT id, email FROM accounts WHERE email = $1', [normalizedEmail])
  ...
  router.post('/auth/reset-password', async (req: AuthRequest, res: Response) => {
    try {
