...
const GROWTH_PRICE_ID =
  process.env.STRIPE_GROWTH_PRICE_ID || "price_1TGrdf2SWVrwMtPK9XWZHZAx";
...
function getPlanKey(subscription: any): string {
  ...
  if (priceId) {
    console.warn(
      `[billing] Unrecognized Stripe price ID: ${priceId}, falling back to amount-based detection`,
    );
  }
...
async function getAiInsightsThisMonth(
  pool: Pool,
  userId: string,
): Promise<number> {
...
export function createCheckFeatureAccess(pool: Pool) {
  return async function checkFeatureAccess(
    ...
    return {
      ...
      reason:
        remaining === 0
          ? "Monthly limit reached. Upgrade to Growth for unlimited."
          : undefined,
    };
  };
}
...
export function createBillingRoutes(
      ...
      try {
        ...
        if (email) {
          try {
            ...
            console.error(
              "[billing/status] Stripe lookup failed, defaulting to free:",
              err,
            );
            stripeHealthy = false;
          }
        ...
        const entitlements = features.map((key) => {
          ...
          if (key === "ai_insights") {
            return {
              ...
              remaining:
                limit === null ? null : Math.max(0, limit - insightsUsed),
        ...
        const plans = [
          {
            plan: {
              ...
              name: "Free",
              description:
                "Track AI costs across every model and provider. Includes 5 AI-powered insights per month.",
              priceAmount: 0,
            },
            features: PLAN_FEATURES.free.map((k) => ({
              key: k,
              name: k
                .replace(/_/g, " ")
          ...
          },
          {
            plan: {
              ...
              name: "Growth",
              description:
                "Unlimited AI insights, priority support, and everything in Free.",
              priceAmount: 29,
            },
            features: PLAN_FEATURES.growth.map((k) => ({
              key: k,
              name: k
                .replace(/_/g, " ")
                .replace(/\b\w/g, (c) => c.toUpperCase()),
                ...
                subscriptions: [
                  {
                    ...
                    plan: {
                      id: planKey,
                      key: planKey,
                      name: planKey === "growth" ? "Growth" : "Free",
                      priceAmount: planKey === "growth" ? 29 : 0,
                    },
                    ...
                    cancelledAt: subscription.canceled_at
                      ...
                      : null,
                    cancelEffectiveAt: subscription.cancel_at
  ...
  router.post(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        // Free plan — cancel existing subscription at end of period
        if (planId === "free") {
          ...
          if (existingSub) {
            ...
            return res.json({ success: true, changeType: "downgrade" });
          }
  ...
  router.get(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        const result = await deps.checkFeatureAccess(
          req.visitorId!,
      ...
      try {
        ...
        if (req.accountEmail) {
          try {
            ...
            console.error(
              "[billing/usage] Stripe plan lookup failed, defaulting to free:",
              err,
            );
          }
        }
