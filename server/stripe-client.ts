...
function getEncryptionKey(): Buffer {
  const key = process.env.INTEGRATION_ENCRYPTION_KEY || (process.env.NODE_ENV !== 'production' ? 'dev-encryption-key-not-for-production' : '')
  if (!key) {
...
export async function getStripeClientForUser(pool: Pool, userId: string): Promise<Stripe> {
  ...
  if (result.rows.length === 0) {
    throw new Error('No Stripe integration found. Please connect Stripe first.')
  }
