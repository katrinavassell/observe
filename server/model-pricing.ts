...
export async function initModelPricing(pool: Pg.Pool): Promise<void> {
  ...
  `);
  await pool.query(
    `CREATE INDEX IF NOT EXISTS idx_user_tiers_lookup ON user_model_pricing_tiers(user_id, model, min_tokens_million)`,
  );
  ...
  setInterval(
    () => {
      refreshFromSources(pool).catch((err) =>
        console.error("Auto pricing refresh failed:", err),
      );
    },
    24 * 60 * 60 * 1000,
  );
...
async function refreshCache(pool: Pg.Pool): Promise<void> {
  const { rows } = await pool.query(
    "SELECT model, provider, input_cost_per_million, output_cost_per_million FROM model_pricing",
  );
...
export async function getAllPricing(pool: Pg.Pool): Promise<ModelPrice[]> {
  if (Date.now() - cacheLoadedAt > CACHE_TTL_MS) {
    if (!refreshPromise) {
      refreshPromise = refreshCache(pool).finally(() => {
        refreshPromise = null;
      });
    }
...
export async function upsertUserModelPricing(
  ...
  const oldInput =
    existing?.input_cost_per_million ??
    (await getModelPricing(pool, model))?.input_cost_per_million ??
    0;
  const oldOutput =
    existing?.output_cost_per_million ??
    (await getModelPricing(pool, model))?.output_cost_per_million ??
    0;
...
export async function deleteUserModelPricing(
  pool: Pg.Pool,
  userId: string,
  model: string,
): Promise<void> {
  await pool.query(
    "DELETE FROM user_model_pricing WHERE user_id = $1 AND model = $2",
    [userId, model],
  );
}
...
export async function getUserPricingTiers(
  pool: Pg.Pool,
  userId: string,
  model: string,
): Promise<PricingTier[]> {
...
export async function setUserPricingTiers(
  ...
  await pool.query(
    "DELETE FROM user_model_pricing_tiers WHERE user_id = $1 AND model = $2",
    [userId, model],
  );
...
export async function deleteUserPricingTiers(
  pool: Pg.Pool,
  userId: string,
  model: string,
): Promise<void> {
...
export async function calculateCostFromTokens(
  ...
  const outputCost =
    ((outputTokens || 0) * pricing.output_cost_per_million) / 1_000_000;
...
async function fetchFromOpenRouter(): Promise<ModelPrice[]> {
  ...
  for (const m of data.data) {
    ...
    results.push({
      ...
      provider,
      input_cost_per_million:
        Math.round(inputPerToken * 1_000_000 * 1000) / 1000,
      output_cost_per_million:
        Math.round(outputPerToken * 1_000_000 * 1000) / 1000,
    });
  }
  return results;
}
...
async function fetchFromLiteLLM(): Promise<ModelPrice[]> {
  ...
  for (const [key, val] of Object.entries(data)) {
    ...
    results.push({
      ...
      input_cost_per_million:
        Math.round((inputCost || 0) * 1_000_000 * 1000) / 1000,
      output_cost_per_million:
        Math.round((outputCost || 0) * 1_000_000 * 1000) / 1000,
    });
  }
  return results;
}
...
export async function refreshFromSources(
  pool: Pg.Pool,
): Promise<{ updated: number; added: number; source: string }> {
  ...
  for (const fresh of freshPricing) {
    ...
    if (!existing) {
      ...
      const inputChanged =
        Math.abs(
          existing.input_cost_per_million - fresh.input_cost_per_million,
        ) > 0.001;
      const outputChanged =
        Math.abs(
          existing.output_cost_per_million - fresh.output_cost_per_million,
        ) > 0.001;
