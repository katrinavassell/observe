...
<template>
  <Card>
    ...
    <CardContent class="space-y-4">
      <!-- Summary Bar -->
      <div class="grid grid-cols-3 sm:grid-cols-6 gap-4 p-4 bg-muted/50 rounded-lg">
        <div class="text-center">
          <div class="font-semibold text-green-600">{{ formatCurrency(totals.new) }}</div>
          <div class="text-xs text-muted-foreground">New MRR</div>
          <div class="text-xs text-green-600">{{ percentages.new.toFixed(1) }}%</div>
        </div>
        <div class="text-center">
          <div class="font-semibold text-green-600">{{ formatCurrency(totals.expansion) }}</div>
          <div class="text-xs text-muted-foreground">Expansion</div>
          <div class="text-xs text-green-600">{{ percentages.expansion.toFixed(1) }}%</div>
        </div>
        <div class="text-center">
          <div class="font-semibold text-red-600">{{ formatCurrency(totals.contraction) }}</div>
          <div class="text-xs text-muted-foreground">Contraction</div>
          <div class="text-xs text-red-600">{{ percentages.contraction.toFixed(1) }}%</div>
        </div>
        <div class="text-center">
          <div class="font-semibold text-red-600">{{ formatCurrency(totals.churn) }}</div>
          <div class="text-xs text-muted-foreground">Churn</div>
          <div class="text-xs text-red-600">{{ percentages.churn.toFixed(1) }}%</div>
        </div>
        <div class="text-center">
          <div
            class="font-semibold"
          ...
          >
            {{ totals.netNew >= 0 ? '+' : '' }}{{ formatCurrency(totals.netNew) }}
          </div>
          ...
          <div
            ...
            :class="totals.netNew >= 0 ? 'text-green-600' : 'text-red-600'"
          >
            {{ percentages.netNew >= 0 ? '+' : '' }}{{ percentages.netNew.toFixed(1) }}%
          </div>
        </div>
        <div class="text-center">
          <div class="font-semibold text-lg">{{ formatCurrency(totals.total) }}</div>
      ...
      <div v-if="!compact" class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead class="bg-muted/50">
            <tr>
              <th class="text-left py-3 px-4 font-medium text-muted-foreground">Month</th>
              <th class="text-right py-3 px-4 font-medium text-muted-foreground">New MRR</th>
              <th class="text-right py-3 px-4 font-medium text-muted-foreground">Expansion</th>
              <th class="text-right py-3 px-4 font-medium text-muted-foreground">Contraction</th>
              <th class="text-right py-3 px-4 font-medium text-muted-foreground">Churn</th>
              <th class="text-right py-3 px-4 font-medium text-muted-foreground">Net New</th>
              <th class="text-right py-3 px-4 font-medium text-muted-foreground">Total MRR</th>
              <th v-if="showGrowth" class="text-right py-3 px-4 font-medium text-muted-foreground">Growth</th>
            </tr>
          </thead>
            ...
            >
              ...
              <td v-if="showGrowth" class="py-3 px-4 text-right">
                <span
                  ...
                  :class="getGrowthColor(calculateGrowth(month.total, month.previousTotal))"
      ...
      <div class="space-y-2 pt-2 border-t">
        ...
        >
          <div class="w-12 text-xs text-muted-foreground">{{ month.monthShort }}</div>
          <div class="flex-1 flex items-center h-5 bg-muted/30 rounded overflow-hidden">
            <!-- Positive MRR (New + Expansion) -->
            <div
              v-if="month.new + month.expansion > 0"
              :style="{ width: `${Math.min(((month.new + month.expansion) / month.total) * 100, 100)}%` }"
              class="h-full bg-green-500"
              :title="`New+Expansion: ${formatCurrency(month.new + month.expansion)}`"
            />

            <!-- Negative MRR (Contraction + Churn) -->
            <div
              v-if="month.contraction + month.churn > 0"
              :style="{ width: `${Math.min(((month.contraction + month.churn) / month.total) * 100, 100)}%` }"
              class="h-full bg-red-500"
              :title="`Contraction+Churn: -${formatCurrency(month.contraction + month.churn)}`"
            />

            <!-- Base MRR (remaining) -->
            <div
              class="h-full bg-gray-300 dark:bg-gray-600 flex-1"
              title="Existing MRR"
            />
          </div>
