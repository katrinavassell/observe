...
/**
 * Shape of a recommendation row returned by GET /api/v1/recommendations.
 *
...
export async function listRecommendations(
  ...
  const res = await request<ListResponse | Recommendation[]>(
    `/v1/recommendations?status=${encodeURIComponent(status)}`,
  );
...
export async function countPendingRecommendations(): Promise<number> {
  const res = await request<{ count: number }>("/v1/recommendations/count");
  return res.count ?? 0;
}

export async function computeRecommendations(): Promise<{ count: number }> {
  return request("/v1/recommendations/compute", { method: "POST" });
}

export async function applyRecommendation(id: number): Promise<void> {
  await request(`/v1/recommendations/${id}/apply`, { method: "POST" });
}

export async function dismissRecommendation(id: number): Promise<void> {
  await request(`/v1/recommendations/${id}/dismiss`, { method: "POST" });
}
 ...
 */
export function recommendationCustomerKey(
  rec: Recommendation,
): string | null {
