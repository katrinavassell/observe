...
const isOnPaidPlan = computed(() => paidPlans.value.some((p: any) => p.key === currentPlanKey.value))
...
const plans = computed(() => {
  ...
  return raw
    .map((p: any) => p.plan ? { ...p.plan, features: p.features } : p)
    .sort((a: any, b: any) => Number(a.priceAmount ?? 0) - Number(b.priceAmount ?? 0))
})

const freePlan = computed(() => plans.value.find((p: any) => Number(p.priceAmount ?? 0) === 0))
const paidPlans = computed(() => plans.value.filter((p: any) => Number(p.priceAmount ?? 0) > 0))
...
async function handleCancel(mode: 'IMMEDIATE' | 'END_OF_PERIOD' = 'END_OF_PERIOD') {
  ...
  <div class="space-y-6">
    ...
    <template v-else-if="!isConfigured">
      <div class="rounded-xl border bg-card p-8 text-center">
        ...
        <p class="text-sm text-muted-foreground mt-2 max-w-md mx-auto">
          Stripe is not configured. All features are currently available without limits.
        </p>
      </div>
    </template>
    ...
    <template v-else>
      ...
      <div v-if="plans.length > 0" class="space-y-8">
        <div class="grid gap-6 max-w-5xl" :class="plans.length === 2 ? 'sm:grid-cols-2 max-w-3xl' : `sm:grid-cols-2 lg:grid-cols-${Math.min(plans.length, 3)}`">
          <div
            v-for="plan in plans"
            :key="plan.id"
            :class="[
          ...
          >
            ...
            <div class="mt-4 mb-5">
              <span class="text-4xl font-bold tracking-tight">${{ plan.priceAmount || 0 }}</span>
              <span class="text-sm text-muted-foreground ml-1">{{ Number(plan.priceAmount ?? 0) === 0 ? 'forever' : '/ month' }}</span>
            </div>

            <ul class="space-y-2.5 mb-6 flex-1">
              <li v-for="f in plan.features" :key="f.key" class="flex items-start gap-2 text-sm">
                <Zap v-if="f.pricingType === 'usage_based'" class="h-4 w-4 text-primary mt-0.5 shrink-0" />
                <Check v-else class="h-4 w-4 text-success mt-0.5 shrink-0" />
                {{ f.name }}
                <span v-if="f.pricing?.maxUsage" class="text-muted-foreground">({{ f.pricing.maxUsage }}/mo)</span>
              </li>
            </ul>

            <!-- Current plan -->
            <template v-if="currentPlanKey === plan.key">
              <div class="w-full rounded-lg border border-success/30 bg-success/10 px-4 py-2.5 text-sm font-medium text-success text-center">
                <Check class="h-4 w-4 inline mr-1" />
                Your current plan
              </div>
              <div v-if="billingPeriodEnd && Number(plan.priceAmount ?? 0) > 0" class="mt-3 text-xs text-muted-foreground text-center">
                Renews {{ billingPeriodEnd }}
              </div>
              <button
                v-if="Number(plan.priceAmount ?? 0) > 0 && !hasScheduledCancellation"
            ...
            <button
              v-else-if="Number(plan.priceAmount ?? 0) === 0 && isOnPaidPlan && !hasScheduledCancellation"
            ...
            <button
              v-else-if="Number(plan.priceAmount ?? 0) > Number(currentPlanPrice)"
            ...
            <button
              v-else-if="Number(plan.priceAmount ?? 0) > 0 && Number(plan.priceAmount ?? 0) < Number(currentPlanPrice)"
              class="w-full rounded-lg border border-border bg-card px-4 py-2.5 text-sm font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              :disabled="isPending"
              @click="handleSubscribe(plan.key)"
            >
      ...
      <div v-else-if="plans.length === 0" class="rounded-xl border bg-card p-8 text-center">
        <AlertCircle class="h-10 w-10 text-muted-foreground mx-auto mb-4" />
