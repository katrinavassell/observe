  ...
  <div class="flex items-center justify-center min-h-screen bg-background">
    <div class="max-w-md w-full mx-auto p-8">
      ...
      <div v-if="isLoading" class="text-center py-8">
        <div class="animate-spin h-8 w-8 border-2 border-primary border-t-transparent rounded-full mx-auto mb-3" />
      ...
      <div v-else-if="inviteInfo" class="space-y-6">
        <div class="border rounded-lg p-5 space-y-3">
          <div>
            <p class="text-xs text-muted-foreground uppercase font-semibold tracking-wider">Workspace</p>
            <p class="text-lg font-semibold">{{ inviteInfo.org_name }}</p>
          </div>
          <div>
            <p class="text-xs text-muted-foreground uppercase font-semibold tracking-wider">Your role</p>
            <p class="capitalize font-medium">{{ inviteInfo.role }}</p>
          </div>
          <div v-if="inviteInfo.invited_email">
            <p class="text-xs text-muted-foreground uppercase font-semibold tracking-wider">Invited for</p>
        ...
        <div class="text-sm text-muted-foreground text-center">
          <template v-if="inviteInfo.role === 'viewer'">
            As a <strong>Viewer</strong>, you'll be able to see all the workspace data: dashboards, customers, pricing, and analytics — but cannot modify data or manage the team.
          </template>
          <template v-else>
            As an <strong>Admin</strong>, you'll have full access to manage data, invite teammates, and configure integrations.
          </template>
        </div>
        ...
        <button
          ...
          @click="acceptInvite"
        >
          <div v-if="isJoining" class="animate-spin h-4 w-4 border-2 border-current border-t-transparent rounded-full" />
